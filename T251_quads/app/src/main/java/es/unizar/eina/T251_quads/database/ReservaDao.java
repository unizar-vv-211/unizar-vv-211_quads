...
/**
 * Data Access Object (DAO) para la entidad Reserva y la gestión de sus
 * relaciones.
 * Se ha definido esta interfaz para especificar los métodos de acceso a los
 * datos
 * (CRUD y consultas complejas) de la tabla reserva_table y la tabla de unión.
 * La implementación de los métodos es generada automáticamente por la librería
 * Room.
 */
@Dao
public interface ReservaDao {
     ...
     * @return El ID (long) de la fila que se ha insertado, lo que permite asociar
     *         las relaciones M:N posteriormente.
     */
    @Insert
     ...
     * Este método se utiliza principalmente para tareas de inicialización o
     * pruebas.
     */
     ...
     * Se ha definido la estrategia {@code OnConflictStrategy.REPLACE} para evitar
     * duplicados
     * en caso de que la relación (clave primaria compuesta) ya exista.
     *
     * @param relacion El objeto de unión {@code RelacionReservaQuad} que se ha de
     *                 insertar.
     */
    ...
    /**
     * Se han eliminado todas las relaciones de quads asociadas a una reserva
     * específica.
     * Este método se ha diseñado para ser invocado durante el proceso de
     * actualización
     ...
     * {@code @Transaction} ha sido utilizado para asegurar que la operación (que
     * consulta múltiples tablas)
     * se ejecute de forma atómica y consistente. Los resultados se han ordenado
     * ascendentemente por fecha de recogida.
     *
    ...
    /**
     * Se han obtenido las reservas que se solapan con un rango de fechas dado para
     * un quad específico.
     * Esta consulta ha sido diseñada para verificar la disponibilidad de un quad
     * antes de crear o modificar una reserva.
     * Se ha utilizado una unión (INNER JOIN) con la tabla de relación para filtrar
     * por quad.
     * La lógica de solapamiento verifica que NO se cumplan las condiciones de
     * no-solapamiento:
     * - La fecha de devolución de la reserva existente es anterior al inicio del
     * rango solicitado, O
     * - La fecha de recogida de la reserva existente es posterior al fin del rango
     * solicitado.
     *
     * @param matricula   La matrícula del quad que se ha de verificar.
     * @param fechaInicio La fecha de inicio del rango solicitado (formato
     *                    YYYY-MM-DD).
     * @param fechaFin    La fecha de fin del rango solicitado (formato YYYY-MM-DD).
     * @param reservaId   El ID de la reserva actual (para excluirla en modo
     *                    edición, usar -1 en modo creación).
